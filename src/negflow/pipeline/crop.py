...
def detect_frame_boundaries(
    ...
    for strip_index, (x0, x1) in enumerate(strip_segments, start=1):
        ...
        content_end, tail_trim = _trim_low_detail_vertical_tail(
            source_strip_luminance,
            content_start,
            content_end,
            strip_width=x1 - x0,
        ...
        for frame_index, (y0, y1) in enumerate(frame_segments, start=1):
            ...
            source_patch_luminance = source_strip_luminance[y0:y1]
            detection_patch_luminance = strip_luminance[y0:y1]
            boxes.append(
                {
                    ...
                    "mean_luminance": float(source_patch_luminance.mean()),
                    "luminance_std": float(source_patch_luminance.std()),
                    "detection_mean_luminance": float(detection_patch_luminance.mean()),
                    "detection_luminance_std": float(detection_patch_luminance.std()),
                    "height": int(y1 - y0),
    ...
    boxes, rejected_boxes = _filter_near_black_boxes(boxes)
    ...
    result = {
        ...
        "notes": [
            ...
            "Long low-detail blank tails are trimmed before estimating frame count.",
            "Unusually wide strips are capped by strip width to avoid over-splitting internal image texture.",
            "Near-black candidate boxes are rejected when at least one brighter candidate remains.",
        ],
    }
...
def _filter_near_black_boxes(
    ...
    *,
    ...
    rejected = []
    for box in boxes:
        mean_luminance = float(box.get("mean_luminance", 0.0))
        if mean_luminance <= max_mean_luminance:
            rejected_box = copy.deepcopy(box)
            rejected_box["rejection_reason"] = "near_black_candidate"
            rejected_box["near_black_mean_luminance_threshold"] = float(max_mean_luminance)
            rejected.append(rejected_box)
        else:
            kept.append(box)
...
def _trim_low_detail_vertical_tail(
    source_strip_luminance: np.ndarray,
    content_start: int,
    content_end: int,
    *,
    strip_width: int,
    ...
    strip = source_strip_luminance[content_start:content_end]
    if strip.size == 0:
        return content_end, detail
    ...
    min_tail_height = max(int(round(min_frame_height * 1.5)), int(round(strip_width * 0.5)))
...
def _estimate_frame_count(
    ...
    estimated = max(1, int(round(content_height / max(float(min_frame_height), typical_height))))

    if strip_width and strip_width >= int(min_frame_height * 2.0):
        plausible_frame_height = max(float(min_frame_height), float(strip_width) * 1.1)
        width_based_cap = max(1, int(round(content_height / plausible_frame_height)))
        estimated = min(estimated, width_based_cap)
