...
def convert_fff_to_tiff(request: FffConversionRequest) -> FffConversionResult:
    ...
    if request.backend_mode == "tiff_passthrough" and not request.converter_command:
        _validate_tiff_compatible_fff(request.input_path)
        return FffConversionResult(
            output_tiff_path=request.input_path,
            backend_mode=request.backend_mode,
            conversion_method="tiff_compatible_reference",
            command_template="tiff_passthrough",
            expanded_command="",
            returncode=0,
            stdout="",
            stderr="",
    ...
    return FffConversionResult(
        ...
        backend_mode=request.backend_mode,
        conversion_method="external_converter",
...
def _validate_tiff_compatible_fff(input_path: Path) -> None:
    try:
        with tifffile.TiffFile(input_path) as tif:
            if not tif.pages:
                raise ValueError("file has no TIFF image pages")
            page = tif.pages[0]
            if page.ndim not in {2, 3}:
                raise ValueError(f"unsupported TIFF page shape: {page.shape}")
    except Exception as exc:
        raise FffBackendUnavailable(
            "FFF input is not directly TIFF-compatible. Configure an external converter command before processing this file."
        ) from exc
